...
import 'package:todo_fui/domain/usecases/auth/sign_up_with_email_and_password_use_case.dart';
...
class RegistrationBloc extends Bloc<RegistrationEvent, RegistrationState> {
  ...
  RegistrationBloc({required SignUpWithEmailAndPasswordUseCase signUpWithEmailAndPasswordUseCase})
  ...
  Future<void> _onRegistrationRequest(
    ...
    result.map(
      onSuccess: (_) => emit(state.copyWith(status: StateStatus.success)),
      onFailure: (error) {
        emit(
          state.copyWith(
